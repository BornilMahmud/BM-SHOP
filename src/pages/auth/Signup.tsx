...
export default function Signup() {
  const { signUp, signInGoogle, signInGithub, error, user, role, loading, clearError } = useAuth();
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-bg-900">
      <div className="w-full max-w-md glass p-8">
        <div className="flex flex-col items-center gap-2 mb-6">
          ...
          <p className="text-sm text-ink-300">Join BM SHOP in seconds</p>
        </div>
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-3">
          ...
          <label className="text-xs text-ink-300">Confirm password
            <input
              type="password"
              required
              minLength={6}
              value={confirm}
              onChange={(e) => { setLocalError(null); setConfirm(e.target.value); }}
              className="input mt-1"
          ...
          <button type="submit" disabled={busy !== null} className="btn-primary w-full mt-1">
            {busy === "email" ? <Loader2 size={16} className="animate-spin" /> : <UserPlus size={16} />}
            Create account
          </button>
        </form>
        ...
        <div className="mt-6 text-xs text-ink-300 text-center">
          ...
          <Link to="/login" className="text-brand-300 hover:underline">Sign in</Link>
        </div>
      </div>
    </div>
  );
